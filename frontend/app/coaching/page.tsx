...
export default function CoachingPage() {
  ...
  return (
    <main>
      ...
      <div className="mb-8 rounded-2xl border border-gray-700 bg-gray-900 p-5">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">今週のセッション</p>
        {currentSession ? (
          currentSession.status === "in_progress" ? (
            <div>
              ...
              <button
                ...
                className="w-full py-3 bg-white hover:bg-gray-200 text-black font-semibold rounded-xl transition-colors"
              >
                セッションを続ける
              </button>
            </div>
        ...
        {addGoalOpen && (
          <form onSubmit={handleAddGoal} className="mb-3 flex gap-2">
            <input
              ...
              autoFocus
              className="flex-1 bg-gray-900 border border-gray-700 rounded-xl px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-gray-500"
            />
            <button
              type="submit"
              disabled={!newGoalTitle.trim() || addingGoal}
              className="px-4 py-2 bg-white text-black text-sm rounded-xl hover:bg-gray-200 disabled:opacity-50"
            >
              追加
            </button>
            <button
            ...
            {goals.map((goal) => (
              <li key={goal.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-900 border border-gray-800">
                ...
                <button
                  ...
                  className="flex-shrink-0 w-6 h-6 rounded-full border border-gray-600 hover:bg-gray-500 hover:border-gray-500 transition-colors flex items-center justify-center"
                  title="完了にする"
                >
