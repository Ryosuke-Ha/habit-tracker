...
以下を返してください:
1. データから読み取れる、本人が気づいていない可能性があるパターンや矛盾（1〜2点）
2. それに基づいた問い（答えを与えず、考えさせる問いを1〜2個）

※ アドバイスや解決策は不要です。問いだけを返してください。
"""
    ...
    message = client.messages.create(
        ...
        max_tokens=800,
        system="""あなたは認知科学ベースのコーチです。
ユーザーが自分で気づいていないパターンや矛盾を指摘し、
「なぜそうなったか」「本当に避けたいことは何か」を
問いかけによって引き出してください。
答えを与えず、気づきを促す問いを最大2つ返してください。""",
...
以下のルールを厳守してアウトプットを生成してください。

【絶対に守るルール】
1. アドバイス・改善案を提示しない
2. 評価・判断をしない
3. 事実ベースのパターンを1〜2個指摘するのみ
4. 最後に「なぜそうなったと思いますか？」という問いを1つだけ投げる
5. ユーザーが自分で来月の行動を決められるよう余白を残す
...
"""
    ...
    message = client.messages.create(
        model="claude-haiku-4-5-20251001",
        max_tokens=800,
        system="""あなたは習慣分析の専門家です。
アドバイスや改善案を提示せず、事実ベースのパターン観察と問いかけのみを行ってください。
ユーザーが自分で気づき、自分で決められるよう余白を残してください。
日本語で回答してください。""",
        messages=[{"role": "user", "content": user_message}],
    )
